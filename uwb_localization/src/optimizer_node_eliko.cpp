...
public:
    ...
    anchor_positions_ = {
        {"0x0009D6", {0.28, -0.24, 0.0}}, {"0x0009ES", {0.28, 0.24, 0.0}},
        {"0x0016FA", {-0.28, 0.24, 0.0}}, {"0x0016CF", {-0.28, 0.24, 0.0}}
    };
    ...
    t_ = {0.0, 0.0, -1.0};       // Translation That_ts (x, y, z)
...
private:
  ...
  void distances_coords_cb_(const eliko_messages::msg::DistancesList::SharedPtr msg) {
      // Clear old measurements each cycle
        current_measurements_.clear();
        
        RCLCPP_INFO(this->get_logger(), "[Eliko optimizer node] Got %ld measurements", msg->anchor_distances.size());
  ...
  struct UWBResidual {
    ...
    eliko_messages::msg::AnchorCoordsList anchors_coords_;
    eliko_messages::msg::TagCoordsList tags_coords_;
