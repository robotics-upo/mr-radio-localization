...
private:
    ...
    void pcl_viewer_cb_(){
        // Keep the viewer running until it is closed manually.
        if (viewer_ && !viewer_->wasStopped()) {
            viewer_->spinOnce(10);
        }
    }

    void updateVisualizer(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &source_cloud,
             const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &target_cloud, 
             const std::string &source_cloud_id, const std::string &target_cloud_id) const {
                
        if (!viewer_ || !source_cloud || !target_cloud) return;

        // Set up a color handler for each cloud
        pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> 
            source_color(source_cloud, 255, 0, 0);  // Red for AGV
        pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> 
            target_color(target_cloud, 0, 255, 0);  // Green for UAV

        // Check if the clouds are already in the viewer
      ...
      std::shared_ptr<UpdatePointClouds::Response> response)
        {
            ...
            updateVisualizer(source_cloud_, target_cloud_, "source_cloud", "target_cloud");
