#ifndef UWB_LOCALIZATION_RESIDUALS_H_
#define UWB_LOCALIZATION_RESIDUALS_H_
...
namespace uwb_localization
{
   // Residual struct for Ceres
    ...
    struct PriorResidual {
        PriorResidual(const Sophus::SE3d& prior_T, const double &roll, const double &pitch, const Eigen::Matrix4d& prior_covariance)
        ...
        static ceres::CostFunction* Create(const Sophus::SE3d& prior_T, const double& roll, const double &pitch, const Eigen::Matrix4d& prior_covariance) {
            return new ceres::AutoDiffCostFunction<PriorResidual, 4, 4>(
                new PriorResidual(prior_T, roll, pitch, prior_covariance));
        }
